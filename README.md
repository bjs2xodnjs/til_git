...
- 나는 main 에서 작업을 하고 있었습니다.
  `login 브랜치`에서 나는 고르인 기능을 구현했다.
...
- 만약 til_git 프로젝트 생성한다면 GitHub 에도 생성하자.
...
- `add` : 추가하라
- `origin`
  - http 주소를 간략하게 별칭을 준 것입니다. (단어는 마음대로 하셔도 되요.)
  - 단어는 마음대로 하셔도 되요.
  - `원격 이름`을 말함.

```bash
git remote add gogogo https://github.com/아이디/til_git.git
```
...
- 이후로는 `-u` 옵션을 붙이셨다면 이후로는 `git puch` 하면 됨.
...
- gitHub 사이트에 Repository `clone` 한다.
